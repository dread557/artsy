...
const Header = () => {
    ...
    return (
        <div className='flex justify-between items-center pt-8 md:pt-[50px] pl-[30px] pr-[30px] md:pl-[65px] md:pr-[65px] lg:pl-[120px] lg:pr-[120px]'>
            <span className='text-2xl md:hidden cursor-pointer' onClick={() => { setActiveMenu(!activeMenu); setMobileMenu(!mobileMenu) }}> <AiOutlineMenu /></span>
            {(mobileMenu && screenSize <= 800) ? (<NavMenu />) : null}
            ...
            <div className='flex gap-[11px]'>
                ...
                {screenSize >= 800 ? (<IoMdNotificationsOutline className='w-[28px] h-[28px] text-[#333333]' cursor-pointer />) : null}
            </div>
